...
![Astra landing page](docs/assets/hero.png)

<p align="center"><em>AI-powered Vedic astrology — birth-chart readings, daily horoscopes, and chart-grounded chat. Screenshots are from the running app (pre-launch; not yet deployed to a public domain).</em></p>
...
![Astra daily horoscope page](docs/assets/horoscope.png)

<p align="center"><em>Daily horoscope — the reading text is generated by the LLM layer and grounded in the sign's transiting placements, not canned copy.</em></p>
...
```
app/            # Next.js App Router — marketing, dashboard, chart, horoscope, chat, API routes
components/     # UI: home/ (marketing), chart/, layout/, dashboard/
lib/            # Supabase clients, engine client, prompt scaffolding, helpers
engine/         # FastAPI service: Vedic calculator, transits, Moon Brief, compatibility
  app/          #   endpoints + calculation modules
  tests/        #   214 pytest cases (deterministic chart math)
supabase/       # 9 SQL migrations (profiles, charts, sessions, subscriptions)
e2e/            # Playwright smoke + authenticated-flow specs
middleware.ts   # auth + subscription gating on every request
```
